...
class Market:
    def __init__(self, spaceship: Spaceship = None):
        ...
        if spaceship is not None:
            current_items = spaceship.current_items
        ...
        self.possible_ship_items = {
            "shield": 200,
            "bullet_power": 100
        }

        self.current_planet = None
        self.planet_item_prices = dict()
        self.buttons = dict()
        ...
        self.sizes = {
            ...
            # TODO CONTINUE HERE
            "SHIP_MARKET_UNIT_PRICE_X": Constants.WINDOW_WIDTH / 2 + self.sizes["s"]
        }

        self.generate_planet_market(possible_planets, possible_items)
        self.draw_market()
    ...
    def generate_planet_market(self, possible_planets: list,
        ...
        for item in current_planet_items:
            self.planet_item_prices[item] = int(possible_items[item] *
                                                random.randint(1, 10))
    ...
    def draw_apply_button(self):
        ...
        APPLY_BUTTON_TOP_LEFT = [
            Constants.WINDOW_WIDTH / 2 - APPLY_BUTTON_DIM[0] / 2,
    ...
    def update_headers(self, screen):
        header_items = self.items_to_draw["headers"]

        item_name_label = header_items["item_name_label"]
        screen.blit(item_name_label[0], item_name_label[1])

        unit_price_label = header_items["unit_price_label"]
        screen.blit(unit_price_label[0], unit_price_label[1])

        current_item_count_label = header_items["current_item_count_label"]
        screen.blit(current_item_count_label[0], current_item_count_label[1])

        # sell_item_label = header_items["sell_item_label"]
        # screen.blit(sell_item_label[0], sell_item_label[1])
        #
        # buy_item_label = header_items["buy_item_label"]
        # screen.blit(buy_item_label[0], buy_item_label[1])
    ...
    def update_gui(self, screen, event=None):
        ...
        # update headers
        self.update_headers(screen)

        for item in self.planet_item_prices:
    ...
    def draw_headers(self, header_y_pos: int) -> int:
        ...
        #     header_y_pos
        # ]
        # header_items["sell_item_label"] = [sell_item_label, sell_item_label_pos]
        #
    ...
    def draw_market(self):
        ...
        for item_i, item in enumerate(self.planet_item_prices):
            ...
            unit_price_button = Button(f"${self.planet_item_prices[item]}",
            ...
            current_item_count_button = Button(
                f"{self.current_items.get(item, 0)}",
                Constants.FONTS["HEADING_2_FONT"],
    ...
    def draw_ship_market(self, first_button_y_pos: int):
        ...
        for item_i, (item, item_price) in \
                enumerate(self.possible_ship_items.items()):
            button_y_pos = first_button_y_pos + button_y_variable * item_i
    ...
    def sell_onClick(self, item: str):
        self.order_cart[item] = self.order_cart.get(item, 0) - 1
        self.change_in_cash += self.planet_item_prices[item]
        self.refurbish_cart_button(item)

    def buy_onClick(self, item: str):
        self.order_cart[item] = self.order_cart.get(item, 0) + 1
        self.change_in_cash -= self.planet_item_prices[item]
        self.refurbish_cart_button(item)
    ...
    def refurbish_cart_button(self, item):
        ...
        current_item_count = \
            self.current_items.get(item, 0) + self.order_cart.get(item, 0)
        if current_item_count > self.current_items.get(item, 0):
            current_item_count_button.update_bg_color(Colors.GREEN)
        elif current_item_count == self.current_items.get(item, 0):
